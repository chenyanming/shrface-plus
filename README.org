...
target is for any non-org buffers. It does not need =shr= to work, =shr= is only for
eww/nov.el/mu4e, so I change to a new name - =inherit-org=, for chasing the
project target: inherit org faces for any non-org buffers.
